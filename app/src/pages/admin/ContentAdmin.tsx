...
import {
  batchContent, changeContentStatus, deleteContentForever, duplicateContent, loadAdminContent, loadAdminContents,
...
export function ContentListPage({ profile }: { profile: Profile }) {
  ...
  const runDeleteForever = async (item: ContentItem) => { try { const result = await deleteContentForever([{ id: item.id, version: item.version }]); notify(`已彻底删除 ${result.succeeded} 篇资料。`, result.succeeded !== 1 || Boolean(result.storageWarnings?.length)); refresh(); } catch (error) { notify(messageOf(error, "彻底删除失败"), true); } };
...
export function ContentEditorPage({ profile }: { profile: Profile }) {
  ...
  const importFile = async (file: File) => { setImporting(true); try { stageImport(await readDocument(file), file); } catch (error) { notify(messageOf(error, "文档读取失败"), true); } finally { setImporting(false); } };
  const importPage = async () => { if (!importUrl.trim()) return; setImporting(true); try { stageImport(await readWebPage(importUrl.trim())); } catch (error) { notify(`${messageOf(error, "网页读取失败")}。腾讯文档请下载 Word 后导入。`, true); } finally { setImporting(false); } };
