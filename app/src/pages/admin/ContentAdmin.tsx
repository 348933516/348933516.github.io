...
import { DocumentOutline } from "../../components/DocumentNavigation";
...
import { defaultOutlineSettings } from "../../lib/outline";
...
export function NewContentPage({ profile }: { profile: Profile }) {
  ...
  const create = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); try { const data = await saveContent({ slug: slugify(title), categoryId, title: title.trim(), summary: "", bodyHtml: "<p></p>", bodyJson: {}, bodyText: "", sourceRecord: "", status: "draft", featured: false, sortOrder: 100, tags: [], outlineEnabled: false, outlineSettings: defaultOutlineSettings }, profile.id); navigate(`/admin/contents/${data.id}`, { replace: true }); } catch (error) { setMessage(messageOf(error, "草稿创建失败")); setSaving(false); } };
...
export function ContentEditorPage({ profile }: { profile: Profile }) {
  ...
  useEffect(() => {
    ...
    void readEditorRecovery(id).then((saved) => {
      if (active && saved?.version === item.version) setRecovery({ ...initial, ...saved.draft, outlineEnabled: saved.draft.outlineEnabled ?? initial.outlineEnabled, outlineSettings: saved.draft.outlineSettings || initial.outlineSettings });
    });
    return () => { active = false; };
  }, [content.data, id]);
...
function DraftPreview({ draft, item, media }: { draft: ContentDraft; item: ContentItem; media: ContentMedia[] }) {
  const document = useMemo(() => prepareRichDocument(draft.bodyHtml, media, draft.outlineSettings), [draft.bodyHtml, draft.outlineSettings, media]);
  const outline = draft.outlineEnabled ? document.outline : [];
  return <article className="draft-preview"><header><span>{item.categoryName}</span><h1>{draft.title}</h1><p>{draft.summary}</p></header><div className={`reader-layout ${outline.length ? "with-outline" : "without-outline"}`}>{outline.length > 0 && <DocumentOutline items={outline} settings={draft.outlineSettings} observe className="reader-document-outline" />}<RichContent html={document.html} prepared /></div></article>;
}
