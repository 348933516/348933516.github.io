...
import {
  ...
  LogOut, Menu, Palette, Plus, RotateCcw, Save, Search, Settings, ShieldCheck, Trash2,
...
import { officialAssets, officialAssetUrl } from "../lib/official-assets";
...
function SettingsPanel({ profile }: { profile: Profile }) {
  ...
  const categories = useAdminCategories();
  const [message, setMessage] = useState("");
  const [assetTarget, setAssetTarget] = useState("hero_background_path");
  ...
  const applyOfficialAsset = async (path: string, name: string) => {
    setApplying(true); setMessage("");
    try {
      if (assetTarget.startsWith("category:")) {
        const categoryId = assetTarget.slice("category:".length);
        const { error } = await supabase.from("categories").update({ image_path: path, updated_by: profile.id }).eq("id", categoryId);
        if (error) throw error;
        client.invalidateQueries({ queryKey: ["admin-categories"] });
      } else {
        const { error } = await supabase.from("site_settings").update({ [assetTarget]: path, updated_by: profile.id }).eq("id", "main");
        if (error) throw error;
      }
      setMessage(`“${name}”已应用。`); refresh();
    } catch (error) { setMessage(messageOf(error, "官方素材应用失败")); }
  ...
  const applyOfficialTheme = async () => {
    ...
    try {
      const { error } = await supabase.from("site_settings").update({
        top_logo_path: "official/maplestory-logo.png",
        hero_logo_path: "official/maplestory-logo.png",
        page_background_path: "official/v226-hero.jpg",
        hero_background_path: "official/v226-hero.jpg",
        tile_background_path: "official/media-khali.png",
        updated_by: profile.id
      }).eq("id", "main");
      if (error) throw error;
      const categoryPaths = ["official/banner-upgrade.jpg", "official/media-ride.png", "official/banner-genesis.jpg", "official/banner-challenge.png"];
      await Promise.all((categories.data || []).slice(0, 4).map((category, index) => supabase.from("categories").update({ image_path: categoryPaths[index], updated_by: profile.id }).eq("id", category.id).then(({ error: categoryError }) => { if (categoryError) throw categoryError; })));
      client.invalidateQueries({ queryKey: ["admin-categories"] });
      setMessage("官方主题已应用到 Logo、背景和前四个类目。"); refresh();
    } catch (error) { setMessage(messageOf(error, "官方主题应用失败")); }
    finally { setApplying(false); }
  };
  const wantsLogo = assetTarget === "top_logo_path" || assetTarget === "hero_logo_path";
  return <><div className="admin-page-heading"><div><span>品牌文字、Logo 和所有界面背景</span><h1>首页设置</h1></div></div><section className="admin-panel"><h2>站点文字</h2><form className="form-grid" onSubmit={submit}><label>站点名称<input name="brandTitle" defaultValue={settings.data.brand_title} /></label><label>顶部副标题<input name="brandSubtitle" defaultValue={settings.data.brand_subtitle} /></label><label>首页标题<input name="heroTitle" defaultValue={settings.data.hero_title} /></label><label>首页说明<input name="heroSubtitle" defaultValue={settings.data.hero_subtitle} /></label><label>类目区标题<input name="categoryTitle" defaultValue={settings.data.category_title} /></label><label>类目区说明<input name="categorySubtitle" defaultValue={settings.data.category_subtitle} /></label><button className="button primary" type="submit"><Save />保存文字</button></form></section><section className="admin-panel"><h2>界面图片</h2><div className="setting-assets">{assets.map(([field, label]) => <SettingAsset key={field} field={field} label={label} current={String(settings.data[field] || "")} userId={profile.id} onSaved={() => { setMessage(`${label}已替换。`); refresh(); }} onMessage={setMessage} />)}</div>{message && <div className="form-message">{message}</div>}</section><section className="admin-panel official-library"><div className="official-library-heading"><div><span>OFFICIAL ASSETS</span><h2>官方素材库</h2><p>素材已保存到本站，不依赖官网图片地址。先选择应用位置，再应用单张素材。</p></div><button className="button primary" type="button" disabled={applying} onClick={applyOfficialTheme}>{applying ? <LoaderCircle className="spin" /> : <Palette />}应用官方主题</button></div><label className="asset-target">应用位置<select value={assetTarget} onChange={(event) => setAssetTarget(event.target.value)}><option value="top_logo_path">顶部 Logo</option><option value="hero_logo_path">首页 Logo</option><option value="page_background_path">全站背景</option><option value="hero_background_path">首页主背景</option><option value="tile_background_path">类目默认背景</option>{(categories.data || []).map((category) => <option value={`category:${category.id}`} key={category.id}>类目：{category.name}</option>)}</select></label><div className="official-asset-grid">{officialAssets.filter((asset) => asset.kind === (wantsLogo ? "logo" : "background")).map((asset) => <article key={asset.id}><img src={officialAssetUrl(asset.path)} alt={asset.name} loading="lazy" /><div><strong>{asset.name}</strong><p>{asset.description}</p><a href={asset.sourceUrl} target="_blank" rel="noreferrer">查看官方来源</a></div><button className="button quiet" type="button" disabled={applying} onClick={() => applyOfficialAsset(asset.path, asset.name)}>应用</button></article>)}</div></section></>;
}
...
function SettingAsset({ field, label, current, userId, onSaved, onMessage }: { field: string; label: string; current: string; userId: string; onSaved(): void; onMessage(value: string): void }) {
  const [progress, setProgress] = useState(0);
  const upload = async (file: File) => { try { if (!file.type.startsWith("image/")) throw new Error("请选择图片文件"); const prepared = await imageToWebp(file); const path = `settings/${field}/${randomId()}-${prepared.name.replace(/[^a-zA-Z0-9._-]/g, "-")}`; await uploadWithProgress(prepared, path, (value) => setProgress(value.percent), undefined, publicMediaBucket); const { error } = await supabase.from("site_settings").update({ [field]: path, updated_by: userId }).eq("id", "main"); if (error) throw error; onSaved(); } catch (error) { onMessage(messageOf(error, "图片上传失败")); } finally { setProgress(0); } };
  const preview = current.startsWith("official/") ? officialAssetUrl(current) : current ? supabase.storage.from(publicMediaBucket).getPublicUrl(current).data.publicUrl : "";
  return <div>{preview ? <img className="setting-asset-preview" src={preview} alt="" /> : <span className="setting-asset-empty"><ImagePlus /></span>}<div><strong>{label}</strong><small>{current || "尚未设置"}</small></div>{progress > 0 ? <span>{progress}%</span> : <label className="small-upload"><Upload />替换<input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) upload(file); event.target.value = ""; }} /></label>}</div>;
}
