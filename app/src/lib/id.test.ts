...
describe("randomId", () => {
  ...
  it("keeps generating a database-safe UUID without a crypto implementation", () => {
    expect(randomId(null, () => 1234, () => 0.5)).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-8[0-9a-f]{3}-[0-9a-f]{12}$/);
  });
});
